...
void KSMShutdownFeedback::slotPaintEffect() {
  ...
  if (KConfigGroup(KGlobal::config(), "Logout").readBoolEntry("doFancyLogout", true)) {
    // doFancyLogoutAdditionalDarkness coeff.
    float l_coeff = (float)KConfigGroup(KGlobal::config(), "Logout").
    ...
    if (m_grayImage.isNull()) {
      ...
      m_grayImage = QPixmap::grabWindow(qt_xrootwin(), 0, 0,
      ...
      while (r != end) {
        *r = *g = *b = (uchar)(((*r)*11 + ((*g)<<4) + (*b)*5) * l_coeff / 32.0f);
        r += 4;
        g += 4;
        b += 4;
      }
...
// having a popup here has severe side effects.
{
  ...
  if (maysd) {
    ...
    if (doUbuntuLogout) {
      ...
      //NOTE: BAD CARMA .. this code is copied line by line from standard konqy dialog
      int def, cur;
      ...
      //NOTE: BAD CARMA .. this code is copied line by line from standard konqy dialog [EOF]
