...
, standAlone(_standAlone)
{
  ...
  QBoxLayout *fLay = new QVBoxLayout(fcsBox->layout(), KDialog::spacingHint());
  ...
  wtstr = i18n(
  "The focus policy is used to determine the active window, i.e."
  ...
  // fLay->addColSpacing(0,QMAX(autoRaiseOn->sizeHint().width(), clickRaiseOn->sizeHint().width()) + 15);

  QWhatsThis::add(autoRaiseOn, i18n(
  ...
  QWhatsThis::add(clickRaiseOn, i18n(
  ...
  QWhatsThis::add(delayFocusOn, i18n(
  "When this option is enabled, there will be a delay after which the"
  " window the mouse pointer is over will become active (receive focus)."));

  QWhatsThis::add(delayFocus, i18n(
  "This is the delay after which the window the mouse pointer is over"
  ...
  " ).\n\nIf this checkbox is checked a popup widget is shown, displaying the"
  " icons of all windows to walk through and the title of the currently"
  " selected one.\n\nOtherwise, the focus is passed to a new window each time"
  " Tab is pressed, with no popup widget. In addition, the previously activated"
  " window will be sent to the back in this mode.");
  ...
  wtstr = i18n("Enable this option if you want keyboard or active desktop"
  " border navigation beyond the edge of a desktop to take you to the opposite"
...
void KFocusConfig::load(void) {
  ...
  else if (key == "FocusFollowsMouse") setFocus(FOCUS_FOLLOWS_MOUSE);
  else if (key == "FocusUnderMouse") setFocus(FOCUS_UNDER_MOUSE);
  else if (key == "FocusStrictlyUnderMouse") setFocus(FOCUS_STRICTLY_UNDER_MOUSE);
...
KAdvancedConfig::KAdvancedConfig(bool _standAlone, KConfig *_config,
...
, standAlone(_standAlone)
{
  ...
  QWhatsThis::add(animateShade, i18n(
  "Animate the action of reducing the window to its titlebar (shading)"
  ...
  QWhatsThis::add(shadeHoverOn, i18n(
  ...
  QWhatsThis::add(electricBox, i18n(
  ...
  QWhatsThis::add(delays, i18n(
  "Here you can set a delay for switching desktops using the active borders"
  " feature. Desktops will be switched after the mouse has been pushed against"
  " a screen border for the specified number of milliseconds."));
  ...
  " both worse and better results than normal level, depending on the applications.</li>"
  ...
  hideUtilityWindowsForInactive = new QCheckBox(
      i18n("Hide utility windows for inactive applications"), this);
  QWhatsThis::add(hideUtilityWindowsForInactive, i18n(
...
int KAdvancedConfig::getElectricBorders() {
  if (active_move->isChecked()) return 1;
  if (active_always->isChecked()) return 2;
  return 0;
}
...
, standAlone(_standAlone)
{
  ...
  QWhatsThis::add(resizeOpaqueOn, i18n(
  "Enable this option if you want a window's content to be shown while resizing"
  " it, instead of just showing a window 'skeleton'. The result may not be"
  " satisfying on slow machines."));

  geometryTipOn = new QCheckBox(i18n(
  "Display window &geometry when moving or resizing"), windowsBox);
  bLay->addWidget(geometryTipOn);
  QWhatsThis::add(geometryTipOn, i18n(
  ...
  minimizeAnimOn = new QCheckBox(i18n("Animate minimi&ze and restore"), windowsBox);
    QWhatsThis::add(minimizeAnimOn, i18n(
...
#undef Below
  ...
  connect(minimizeAnimOn, SIGNAL(toggled(bool)), this,
          SLOT(setMinimizeAnim(bool)));
  connect(minimizeAnimSlider, SIGNAL(valueChanged(int)), this,
          SLOT(setMinimizeAnimSpeed(int)));
  ...
  moveResizeMaximized = new QCheckBox(i18n(
  "Allow moving and resizing o&f maximized windows"), windowsBox);
  bLay->addWidget(moveResizeMaximized);
  QWhatsThis::add(moveResizeMaximized, i18n(
  ...
  BrdrSnap->setSteps(1,10);
  QWhatsThis::add(WndwSnap, i18n("Here you can set the snap zone for windows,"
  " i.e. the 'strength' of the magnetic field which will make windows snap to"
  " each other when they're moved near another window."));

  OverlapSnap=new QCheckBox(i18n("Snap windows onl&y when overlapping"),MagicBox);
  QWhatsThis::add(OverlapSnap, i18n(
...
void KMovingConfig::load(void) {
  ...
  if (key == "Transparent") setMove(TRANSPARENT);
  else if (key == "Opaque") setMove(OPAQUE);
  ...
  if (key == "Opaque") setResizeOpaque(RESIZE_OPAQUE);
  else if (key == "Transparent") setResizeOpaque(RESIZE_TRANSPARENT);
  ...
  if (key == "Random") setPlacement(RANDOM_PLACEMENT);
  else if (key == "Cascade") setPlacement(CASCADE_PLACEMENT); //CT 31jan98
  //CT 31mar98 manual placement
  else if (key == "manual") setPlacement(MANUAL_PLACEMENT);
...
void KMovingConfig::save(void) {
  ...
  v = getMove();
  if (v == TRANSPARENT) config->writeEntry(KWIN_MOVE,"Transparent");
  else config->writeEntry(KWIN_MOVE,"Opaque");

  config->writeEntry(KWIN_GEOMETRY, getGeometryTip());

  // placement policy --- CT 31jan98 ---
  v =getPlacement();
  if (v == RANDOM_PLACEMENT) config->writeEntry(KWIN_PLACEMENT, "Random");
  else if (v == CASCADE_PLACEMENT) config->writeEntry(KWIN_PLACEMENT, "Cascade");
...
// }
  else config->writeEntry(KWIN_PLACEMENT, "Smart");

  config->writeEntry(KWIN_MINIMIZE_ANIM, getMinimizeAnim());
  config->writeEntry(KWIN_MINIMIZE_ANIM_SPEED, getMinimizeAnimSpeed());

  v = getResizeOpaque();
  if (v == RESIZE_OPAQUE) config->writeEntry(KWIN_RESIZE_OPAQUE, "Opaque");
  else config->writeEntry(KWIN_RESIZE_OPAQUE, "Transparent");
...
, standAlone(_standAlone)
{
  ...
  if (!kompmgrAvailable_) {
    ...
    "<qt><b>It seems that alpha channel support is not available.</b><br><br>"
    "Please make sure you have <a href=\"http://www.freedesktop.org/\">Xorg &ge"
    "; 6.8</a>, and installed the kompmgr that came with kwin.<br> Also, make"
    " sure you have the following entries in your XConfig"
    " (e.g. /etc/X11/xorg.conf):<br><br><i>Section \"Extensions\"<br>"
    "Option \"Composite\" \"Enable\"<br>EndSection</i><br><br>And if your GPU "
    "provides hardware-accelerated Xrender support (mainly nVidia cards):<br><br>"
    "<i>Option     \"RenderAccel\" \"true\"</i><br>In <i>Section \"Device\""
    "</i></qt>"), this);
      lay->addWidget(label);
  } else {
    ...
    keepAboveAsActive = new QCheckBox(i18n(
    "Treat 'keep above' windows as active ones"),tGroup);
    vLay->addWidget(keepAboveAsActive);

    disableARGB = new QCheckBox(i18n(
    "Disable ARGB windows (ignores window alpha maps, fixes gtk1 apps)"),tGroup);
    ...
    connect(activeWindowTransparency, SIGNAL(toggled(bool)), activeWindowOpacity,
            SLOT(setEnabled(bool)));
    connect(inactiveWindowTransparency, SIGNAL(toggled(bool)), inactiveWindowOpacity,
            SLOT(setEnabled(bool)));
    connect(movingWindowTransparency, SIGNAL(toggled(bool)), movingWindowOpacity,
            SLOT(setEnabled(bool)));
    connect(dockWindowTransparency, SIGNAL(toggled(bool)), dockWindowOpacity,
            SLOT(setEnabled(bool)));
    ...
    connect(useShadows, SIGNAL(toggled(bool)), dockWindowShadowSize,
            SLOT(setEnabled(bool)));
    connect(useShadows, SIGNAL(toggled(bool)), activeWindowShadowSize,
            SLOT(setEnabled(bool)));
    connect(useShadows, SIGNAL(toggled(bool)), inactiveWindowShadowSize,
            SLOT(setEnabled(bool)));
    connect(useShadows, SIGNAL(toggled(bool)), shadowTopOffset,
            SLOT(setEnabled(bool)));
    connect(useShadows, SIGNAL(toggled(bool)), shadowLeftOffset,
            SLOT(setEnabled(bool)));
    connect(useShadows, SIGNAL(toggled(bool)), shadowColor,
            SLOT(setEnabled(bool)));
    ...
    connect(useTranslucency, SIGNAL(toggled(bool)), this,
            SLOT(showWarning(bool)));
...
void KTranslucencyConfig::load(void) {
  if (!kompmgrAvailable_) return;

  config->setGroup("Notification Messages");
  useTranslucency->setChecked(config->readBoolEntry("UseTranslucency",false));

  config->setGroup("Translucency");
  activeWindowTransparency->setChecked(config->readBoolEntry(
                                       "TranslucentActiveWindows",false));
  inactiveWindowTransparency->setChecked(config->readBoolEntry(
                                         "TranslucentInactiveWindows",true));
  movingWindowTransparency->setChecked(config->readBoolEntry(
  ...
  useShadows->setChecked(conf_.readEntry("Compmode","CompClientShadows").compare(
                         "CompClientShadows") == 0);
...
void KTranslucencyConfig::save(void) {
  ...
  conf_->writeEntry("ShadowRadius",
    (activeWindowShadowSize->value() + inactiveWindowShadowSize->value()) / 2);
...
void KTranslucencyConfig::showWarning(bool alphaActivated) {
  ...
  " (sometimes the translucency engine, seldom even X).</qt>"), i18n("Warning"));
}
