...
namespace KWinInternal {
  ...
  void DetectDialog::readWindow(WId w) {
    ...
    type = info.windowType(NET::NormalMask|NET::DesktopMask|NET::DockMask|NET::ToolbarMask|NET::MenuMask|
  ...
  void DetectDialog::executeDialog() {
    ...
    if(widget->use_role->isEnabled()) { widget->use_role->setChecked(true); }
    else { widget->use_whole_class->setChecked(true); }
    if(type == NET::Unknown) { widget->type_label->setText(i18n("Unknown - will be treated as Normal Window")); }
    else { widget->type_label->setText(i18n(types[type])); }
  ...
  QCString DetectDialog::selectedClass() const {
    if(widget->use_class->isChecked() || widget->use_role->isChecked()) { return wmclass_class; }
    return wmclass_name + ' ' + wmclass_class;
  }

  bool DetectDialog::selectedWholeClass() const { return widget->use_whole_class->isChecked(); }

  QCString DetectDialog::selectedRole() const {
    if(widget->use_role->isChecked()) { return role; }
    return "";
  }

  QString DetectDialog::selectedTitle() const { return title; }
  ...
  bool DetectDialog::selectedWholeApp() const { return widget->use_class->isChecked(); }

  NET::WindowType DetectDialog::selectedType() const { return type; }

  QCString DetectDialog::selectedMachine() const { return machine; }
  ...
  WId DetectDialog::findWindow() {
    ...
    for(int i = 0; i < 10; ++i) {
      XQueryPointer(qt_xdisplay(), parent, &root, &child, &rootX, &rootY, &x, &y, &mask);
