...
#define SETUP( var, type ) \
  ...
  , detect_dlg(NULL)
  {
    QString enableDesc = i18n(
    "Enable this checkbox to alter this window property for the specified"
    " window(s).");
    QString setRuleDesc = i18n(
    "Specify how the window property should be affected:<ul><li><em>Do Not"
    " Affect:</em> The window property will not be affected and therefore"
    " the default handling for it will be used. Specifying this will block"
    " more generic window settings from taking effect.</li><li><em>Apply"
    ...
    "<li><em>Remember:</em> The value of the window property will be remembered"
    " and every time time the window is created, the last remembered value will"
    " be applied.</li><li><em>Force:</em> The window property will be always"
    " forced to the given value.</li>"
    "<li><em>Apply Now:</em> The window property will be set to the given value"
    " immediately and will not be affected later (this action will be deleted"
    " afterwards).</li><li><em>Force temporarily:</em> The window property will"
    " be forced to the given value until it is hidden (this action will be"
    " deleted after the window is hidden).</li></ul>");
    QString forceRuleDesc = i18n(
    "Specify how the window property should be affected:<ul><li><em>Do Not"
    " Affect:</em> The window property will not be affected and therefore the"
    " default handling for it will be used. Specifying this will block more"
    " generic window settings from taking effect.</li><li><em>Force:</em> The"
    " window property will be always forced to the given value.</li>li><em>"
    "Force temporarily:</em> The window property will be forced to the given"
    ...
    KWinModule module;
    int i;
    for (i=1; i <= module.numberOfDesktops(); ++i) {
...
#undef UPDATE_ENABLE_SLOT
  ...
  static QString positionToStr(const QPoint& p) {
    if (p == invalidPoint) return QString::null;
  ...
  static int typeToCombo(NET::WindowType type) {
    if (type < NET::Normal || type > NET::Splash) return 0; // Normal
    static const int conv[] = {
      0, // Normal
      7, // Desktop
      3, // Dock
...
#define CHECKBOX_SET_RULE( var, func ) \
  GENERIC_RULE( var, func, Set, set, setChecked, setChecked( false ))
#define LINEEDIT_SET_RULE( var, func ) \
  GENERIC_RULE( var, func, Set, set, setText, setText( "" ))
#define COMBOBOX_SET_RULE( var, func ) \
  GENERIC_RULE( var, func, Set, set, setCurrentItem, setCurrentItem( 0 ))
#define CHECKBOX_FORCE_RULE( var, func ) \
  GENERIC_RULE( var, func, Force, force, setChecked, setChecked( false ))
#define LINEEDIT_FORCE_RULE( var, func ) \
  GENERIC_RULE( var, func, Force, force, setText, setText( "" ))
#define COMBOBOX_FORCE_RULE( var, func ) \
  GENERIC_RULE( var, func, Force, force, setCurrentItem, setCurrentItem( 0 ))

  void RulesWidget::setRules(Rules* rules) {
    Rules tmp;
    if (rules == NULL) rules = &tmp; // empty
...
#define CHECKBOX_SET_RULE( var, func ) \
  GENERIC_RULE( var, func, Set, set, isChecked )
#define LINEEDIT_SET_RULE( var, func ) \
  GENERIC_RULE( var, func, Set, set, text )
#define COMBOBOX_SET_RULE( var, func ) \
  GENERIC_RULE( var, func, Set, set, currentItem )
#define CHECKBOX_FORCE_RULE( var, func ) \
  GENERIC_RULE( var, func, Force, force, isChecked )
#define LINEEDIT_FORCE_RULE( var, func ) \
  GENERIC_RULE( var, func, Force, force, text )
#define COMBOBOX_FORCE_RULE( var, func ) \
  GENERIC_RULE( var, func, Force, force, currentItem )
  ...
  Rules* RulesWidget::rules() const {
    ...
    for (uint i=0; i < types->count(); ++i) {
      if (!types->isSelected(i)) all_types = false;
    }
...
#define CHECKBOX_PREFILL( var, func, info ) \
  GENERIC_PREFILL( var, func, info, setChecked )
#define LINEEDIT_PREFILL( var, func, info ) \
  GENERIC_PREFILL( var, func, info, setText )
#define COMBOBOX_PREFILL( var, func, info ) \
  GENERIC_PREFILL( var, func, info, setCurrentItem )
...
#undef COMBOBOX_PREFILL
  ...
  bool RulesWidget::finalCheck()
  {
    if(description->text().isEmpty()) {
      ...
      else description->setText(i18n("Unnamed entry"));
    }
    bool all_types = true;
    for(uint i=0; i < types->count(); ++i) {
    ...
    if(wmclass_match->currentItem() == Rules::UnimportantMatch && all_types) {
      if (KMessageBox::warningContinueCancel(topLevelWidget(), i18n(
  ...
  void RulesWidget::shortcutEditClicked() {
    ...
    if (dlg.exec() == QDialog::Accepted) shortcut->setText(dlg.shortcut());
  }
  ...
  Rules* RulesDialog::edit(Rules* r, WId window, bool show_hints) {
    ...
    if (window != 0) widget->prepareWindowSpecific(window);
    if (show_hints) QTimer::singleShot(0, this, SLOT(displayHints()));
    exec();
    return rules;
  }
  ...
  void EditShortcut::editShortcut() {
    ...
    if (dlg.exec() == QDialog::Accepted) shortcut->setText(dlg.shortcut().toString());
  }
