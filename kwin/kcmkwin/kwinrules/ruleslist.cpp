...
namespace KWinInternal {
  ...
  : KCMRulesListBase(parent, name)
  {
    ...
    connect(rules_listbox, SIGNAL(currentChanged(QListBoxItem*)), SLOT(activeChanged(QListBoxItem*)));
    ...
    connect(rules_listbox, SIGNAL(doubleClicked (QListBoxItem*)), SLOT(modifyClicked()));
    load();
  }

  KCMRulesList::~KCMRulesList() {
    for(QValueVector< Rules* >::Iterator it = rules.begin(); it != rules.end(); ++it) { delete *it; }
    rules.clear();
  }

  void KCMRulesList::activeChanged(QListBoxItem* item) {
    // make current==selected
    if(item != NULL) { rules_listbox->setSelected(item, true); }
  ...
  void KCMRulesList::load() {
    ...
    if(rules.count() > 0) { rules_listbox->setSelected(0, true); }
    else { activeChanged(NULL); }
  }

  void KCMRulesList::save() {
    KConfig cfg("kwinrulesrc");
    QStringList groups = cfg.groupList();
    for(QStringList::ConstIterator it = groups.begin(); it != groups.end(); ++it) { cfg.deleteGroup(*it); }
    cfg.setGroup("General");
    cfg.writeEntry("count", rules.count());
    int i = 1;
    for(QValueVector< Rules* >::ConstIterator it = rules.begin(); it != rules.end(); ++it) {
      cfg.setGroup(QString::number(i));
      (*it)->write(cfg);
      ++i;
    }
