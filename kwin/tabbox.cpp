...
namespace KWinInternal {
  ...
  */
  void TabBox::createClientList(ClientList &list, int desktop /*-1 = all*/, Client *c, bool chain) {
    ...
    if(chain) c = workspace()->nextFocusChainClient(c);
    else c = workspace()->stk_order().first();

    Client* stop = c;

    while(c) {
      Client* add = NULL;
      if(((desktop == -1) || c->isOnDesktop(desktop)) && c->wantsTabFocus()) {
        // don't add windows that have modal dialogs
        Client* modal = c->findModal();
        if (modal == NULL || modal == c) add = c;
        else if (!list.contains(modal)) add = modal;
        else {
          // nothing
        }
      }
      ...
      if(options->separateScreenFocus && options->xineramaEnabled) {
        if(c->screen() != workspace()->activeScreen()) add = NULL;
      }
      ...
      if(chain) c = workspace()->nextFocusChainClient(c);
        ...
        if (idx >= (workspace()->stk_order().size()-1)) c = 0;
        else c = workspace()->stk_order()[++idx];
      }
  ...
  */
  void TabBox::reset() {
    ...
    setGeometry((r.width()-w)/2 + r.x(), (r.height()-h)/2+ r.y(), w, h);
  }
  ...
  */
  void TabBox::nextPrev(bool next) {
    if(mode() == WindowsMode) {
      ...
      if(next) {
        desk++;
        if(desk > workspace()->numberOfDesktops()) desk = 1;
      } else {
        desk--;
        if(desk < 1) desk = workspace()->numberOfDesktops();
      }
    }
    update();
  }
  ...
  void TabBox::drawContents(QPainter *) {
    ...
    if(mode() == WindowsMode) {
      if(!currentClient()) {
        ...
        for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it) {
          if(workspace()->hasClient(*it)) {
            ...
            QPixmap icon;
            if(showMiniIcon) {
              if(!(*it)->miniIcon().isNull()) icon = (*it)->miniIcon();
            } else
              if(!(*it)->icon().isNull()) icon = (*it)->icon();
              else if(menu_pix) icon = *menu_pix;

            if(!icon.isNull()) {
              if((*it)->isMinimized()) KIconEffect::semiTransparent(icon);
            ...
            if((*it)->isMinimized()) s += QString("(") + (*it)->caption() + ")";
            else s += (*it)->caption();
      ...
      for(int i = 1; i <= workspace()->numberOfDesktops(); i++) {
        ...
        // current desktop
        if(iDesktop == desk) p.fillRect(x, y, r.width(), lineHeight, colorGroup().highlight());
        ...
        if(iDesktop == desk) p.setPen(colorGroup().highlightedText());
        ...
        for(ClientList::ConstIterator it = list.fromLast(); it != list.end(); --it) {
          if(!(*it)->miniIcon().isNull()) {
            // only show full icons
            if(x1+18 >= x+r.width()) break;
  ...
  void TabBox::handleMouseEvent(XEvent* e) {
    ...
    if(mode() == WindowsMode) {
      ...
      for(int i = 1; i <= workspace()->numberOfDesktops(); ++i) {
        ...
        num--;
        if(mode() == DesktopMode) iDesktop = workspace()->nextDesktopFocusChain(iDesktop);
        else iDesktop++;
      }
    }
    update();
  ...
  */
  ...
  static bool areModKeysDepressed(const KShortcut& cut) {
    for(unsigned int i = 0; i < cut.count(); ++i) {
      if(areModKeysDepressed(cut.seq(i))) return true;
    }
    return false;
  }

  void Workspace::slotWalkThroughWindows() {
    if(root != qt_xrootwin()) return;
    if(tab_grab || control_grab) return;
  ...
  void Workspace::slotWalkBackThroughWindows() {
    ...
    if(options->altTabStyle == Options::CDE || !options->focusPolicyIsReasonable()) {
      ...
      if(areModKeysDepressed(cutWalkThroughWindowsReverse)) {
        if(startKDEWalkThroughWindows()) KDEWalkThroughWindows(false);
      } else {
  ...
  void Workspace::slotWalkThroughDesktops() {
    ...
    if(areModKeysDepressed(cutWalkThroughDesktops)) {
      if(startWalkThroughDesktops()) walkThroughDesktops(true);
    } else {
  ...
  void Workspace::slotWalkBackThroughDesktops() {
    ...
    if(areModKeysDepressed(cutWalkThroughDesktopsReverse)) {
      if(startWalkThroughDesktops()) walkThroughDesktops(false);
    } else {
  ...
  void Workspace::slotWalkThroughDesktopList() {
    ...
    if(areModKeysDepressed(cutWalkThroughDesktopList)) {
      if(startWalkThroughDesktopList()) walkThroughDesktops(true);
    } else {
  ...
  void Workspace::slotWalkBackThroughDesktopList() {
    ...
    if(areModKeysDepressed(cutWalkThroughDesktopListReverse)) {
      if(startWalkThroughDesktopList()) walkThroughDesktops(false);
    } else {
  ...
  bool Workspace::startKDEWalkThroughWindows() {
    if(!establishTabBoxGrab()) return false;
    tab_grab = TRUE;
    keys->suspend(true);
  ...
  void Workspace::CDEWalkThroughWindows(bool forward) {
    ...
    if(nc) {
      if(c && c != nc) lowerClient(c);
      if(options->focusPolicyIsReasonable()) {
        activateClient(nc);
        if(nc->isShade() && options->shadeHover) nc->setShade(ShadeActivated);
      } else {
        if(!nc->isOnDesktop(currentDesktop())) setCurrentDesktop(nc->desktop());
        raiseClient(nc);
      }
    }
  }
  ...
  void Workspace::KDEOneStepThroughWindows(bool forward) {
    ...
    if(Client* c = tab_box->currentClient()) {
      activateClient(c);
      if(c->isShade() && options->shadeHover) c->setShade(ShadeActivated);
    }
  }
  ...
  void Workspace::oneStepThroughDesktops(bool forward, int mode) {
    ...
    tab_box->reset();
    tab_box->nextPrev(forward);
    if(tab_box->currentDesktop() != -1) setCurrentDesktop(tab_box->currentDesktop());
  }
  ...
  */
  void Workspace::tabBoxKeyPress(const KKeyNative& keyX) {
    ...
    if(tab_grab) {
      ...
      forward = cutWalkThroughDesktops.contains(keyX) || cutWalkThroughDesktopList.contains(keyX);
      backward = cutWalkThroughDesktopsReverse.contains(keyX) || cutWalkThroughDesktopListReverse.contains(keyX);
      if(forward || backward) walkThroughDesktops(forward);
    }
    ...
    if(control_grab || tab_grab) {
      ...
      if(((keyQt & 0xffff) == Qt::Key_Escape) && !(forward || backward)) {
        closeTabBox();
      }
    }
  }
  ...
  */
  void Workspace::tabBoxKeyRelease(const XKeyEvent& ev) {
    ...
    bool release = false;
    if(mod_index == -1) release = true;
    else {
      XModifierKeymap* xmk = XGetModifierMapping(qt_xdisplay());
      for(int i=0; i<xmk->max_keypermod; i++)
          if(xmk->modifiermap[xmk->max_keypermod * mod_index + i] == ev.keycode)
            release = true;
  ...
  int Workspace::nextDesktopFocusChain(int iDesktop) const {
    ...
    if(i >= 0 && i+1 < (int)desktop_focus_chain.size()) return desktop_focus_chain[i+1];
    else if(desktop_focus_chain.size() > 0) return desktop_focus_chain[ 0 ];
    else return 1;
  }
  ...
  */
  Client* Workspace::nextFocusChainClient(Client* c) const {
    if(global_focus_chain.isEmpty()) return 0;
    ClientList::ConstIterator it = global_focus_chain.find(c);
    if(it == global_focus_chain.end()) return global_focus_chain.last();
    if(it == global_focus_chain.begin()) return global_focus_chain.last();
    --it;
    return *it;
  }
  ...
  */
  Client* Workspace::previousFocusChainClient(Client* c) const {
    if(global_focus_chain.isEmpty()) return 0;
    ClientList::ConstIterator it = global_focus_chain.find(c);
    if(it == global_focus_chain.end()) return global_focus_chain.first();
    ++it;
    if(it == global_focus_chain.end()) return global_focus_chain.first();
    return *it;
  }
  ...
  */
  Client* Workspace::nextStaticClient(Client* c) const {
    if(!c || clients.isEmpty()) return 0;
    ClientList::ConstIterator it = clients.find(c);
    if(it == clients.end()) return clients.first();
    ++it;
    if(it == clients.end()) return clients.first();
    return *it;
  }
  ...
  */
  Client* Workspace::previousStaticClient(Client* c) const {
    if(!c || clients.isEmpty()) return 0;
    ClientList::ConstIterator it = clients.find(c);
    if(it == clients.end()) return clients.last();
    if(it == clients.begin()) return clients.last();
    --it;
    return *it;
  }

  bool Workspace::establishTabBoxGrab() {
    if(XGrabKeyboard(qt_xdisplay(), root, FALSE, GrabModeAsync, GrabModeAsync, qt_x_time) != GrabSuccess)
      return false;
    ...
    if(active_client != NULL) active_client->updateMouseGrab();
    return true;
  }
