...
#undef BRIDGE_HELPER
  ...
  void Bridge::setKeepAbove(bool set) {
    if(c->keepAbove() != set) c->workspace()->performWindowOperation(c, KeepAboveOp);
  }

  void Bridge::setKeepBelow(bool set) {
    if(c->keepBelow() != set) c->workspace()->performWindowOperation(c, KeepBelowOp);
  }
  ...
  QIconSet Bridge::icon() const { return QIconSet(c->miniIcon(), c->icon()); }

  bool Bridge::isSetShade() const { return c->shadeMode() != ShadeNone; }

  void Bridge::showWindowMenu(QPoint p) { c->workspace()->showWindowMenu(p, c); }

  void Bridge::showWindowMenu(const QRect &p) { c->workspace()->showWindowMenu(p, c); }

  void Bridge::performWindowOperation(WindowOperation op) { c->workspace()->performWindowOperation(c, op); }

  void Bridge::setMask(const QRegion& r, int mode) { c->setMask(r, mode); }

  bool Bridge::isPreview() const { return false; }
  ...
  QWidget* Bridge::workspaceWidget() const { return c->workspace()->desktopWidget(); }

  WId Bridge::windowId() const { return c->window(); }
  ...
  void Bridge::setShade(bool set) { c->setShade(set ? ShadeNormal : ShadeNone); }

  int Bridge::currentDesktop() const { return c->workspace()->currentDesktop(); }

  QWidget* Bridge::initialParentWidget() const { return NULL; }

  Qt::WFlags Bridge::initialWFlags() const { return 0; }
  ...
  QRegion Bridge::unobscuredRegion(const QRegion& r) const {
    ...
    for(; it != stacking_order.end(); ++it) {
      if(!(*it)->isShown(true)) continue; // these don't obscure the window
      if(c->isOnAllDesktops()) {
        if(!(*it)->isOnCurrentDesktop()) continue;
      }
      else {
        if(!(*it)->isOnDesktop(c->desktop())) continue;
      }
      /* the clients all have their mask-regions in local coords so we have to translate them to a shared coord system
          we choose ours */
      int dx = (*it)->x() - c->x();
      int dy = (*it)->y() - c->y();
  ...
  void Bridge::grabXServer(bool grab) {
    if(grab) { KWinInternal::grabXServer(); }
    else { KWinInternal::ungrabXServer(); }
  }
