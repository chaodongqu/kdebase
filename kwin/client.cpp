...
// SELI do all as initialization
  ...
  */
  void Client::releaseWindow(bool on_shutdown) {
    ...
    if (moveResizeMode) leaveMoveResize();
    finishWindowRules();
    ++postpone_geometry_updates;
    // grab X during the release to make removing of properties,
    // setting to withdrawn state and repareting to root an atomic operation
    // (http://lists.kde.org/?l=kde-devel&m=116448102901184&w=2)
    grabXServer();
    setMappingState(WithdrawnState);
    setModal(false); // otherwise its mainwindow wouldn't get focus
    hidden = true; // so that it's not considered visible anymore (can't use hideClient(), it would set flags)
    if (!on_shutdown) workspace()->clientHidden(this);
  ...
  // like releaseWindow(), but this one is called when the window has been
  // already destroyed (e.g. the application closed it)
  ...
  {
    if (!force && ((decoration == NULL && noBorder()) || (decoration != NULL && !noBorder())))
      return;
    bool do_show = false;
    postponeGeometryUpdates(true);
    if (force) destroyDecoration();
    if (!noBorder()) {
    ...
    if (check_workspace_pos) checkWorkspacePosition();
    postponeGeometryUpdates(false);
    if (do_show) decoration->widget()->show();
    updateFrameExtents();
  }
  ...
  void Client::checkBorderSizes() {
    ...
    if(new_left == border_left &&
        new_right == border_right &&
        new_top == border_top &&
        new_bottom == border_bottom)
      return;
    ...
    if(border_left != new_left || border_right != new_right || border_top != new_top || border_bottom != new_bottom)
    options->onlyDecoTranslucent ? setDecoHashProperty(new_top, new_right, new_bottom, new_left) : unsetDecoHashProperty();
    move(calculateGravitation(false));
    plainResize(sizeForClientSize(clientSize()), ForceGeometrySet);
  ...
  void Client::detectShapable() {
    if (Shape::hasShape(window())) return;
    switch (windowType()) {
      case NET::Desktop :
      case NET::Dock :
      case NET::TopMenu :
  ...
  QRegion Client::mask() const {
    if (_mask.isEmpty()) return QRegion(0, 0, width(), height());
    return _mask;
  }
  ...
  bool Client::isMinimizable() const {
    if (isSpecialWindow()) return false;
    if (isTransient()) {
        //NOTE: #66868 - let other xmms windows be minimized when the mainwindow is minimized
      bool shown_mainwindow = false;
      ClientList mainclients = mainClients();
      for (cl_iter_c it = mainclients.begin(); it != mainclients.end(); ++it) {
        if ((*it)->isShown(true)) shown_mainwindow = true;
      }
      if (!shown_mainwindow) return true;
    }
  ...
  void Client::minimize(bool avoid_animation) {
    if (!isMinimizable() || isMinimized()) return;
  ...
  void Client::animateMinimizeOrUnminimize(bool minimize) {
    ...
    if (!options->animateMinimize) return;

    // decoration did it
    if (decoration != NULL && decoration->animateMinimize(minimize)) return;
    ...
    if (area2 == area || need_to_clear) p.drawPixmap(area2.x(), area2.y(), pm2);
  ...
  void Client::setShade(ShadeMode mode) {
    if (!isShadeable()) return;
    mode = rules()->checkShade(mode);
    if (shade_mode == mode) return;
    bool was_shade = isShade();
    ShadeMode was_shade_mode = shade_mode;
    shade_mode = mode;
    if (was_shade == isShade()) {
      // decoration may want to update after e.g. hover-shade changes
      if (decoration != NULL) decoration->shadeChange();
      return; // no real change in shaded state
    }

    if (shade_mode == ShadeNormal) {
      if (isShown(true) && isOnCurrentDesktop()) Notify::raise(Notify::ShadeUp);
    } else if (shade_mode == ShadeNone) {
      if (isShown(true) && isOnCurrentDesktop()) Notify::raise(Notify::ShadeDown);
    }
    ...
    if (isShade()) { // shade_mode == ShadeNormal
      ...
      long _shade = 1;
      XChangeProperty(qt_xdisplay(), frameId(), atoms->net_wm_window_shade,
                      XA_CARDINAL, 32, PropModeReplace, (uchar*)&_shade, 1L);
      // shade
      int h = height();
      ...
      // if ( !wasStaticContents ) clearWFlags( WStaticContents );
      plainResize(s);
      shade_geometry_change = false;
      if (isActive()) {
        if (was_shade_mode == ShadeHover) workspace()->activateNextClient(this);
        else workspace()->focusToNull();
      }
      ...
      shade_geometry_change = false;
      plainResize(s);
      if (shade_mode == ShadeHover || shade_mode == ShadeActivated) setActive(TRUE);
      ...
      if (isActive()) workspace()->requestFocus(this);
    }
  ...
  void Client::updateVisibility() {
    ...
    if (show) info->setState(0, NET::Hidden);
    ...
    if (show) {
      ...
      for (cl_iter_c it = group()->members().begin(); it != group()->members().end(); ++it)
        if ((*it)->isDesktop()) {
      ...
      if (isShade()) setMappingState(IconicState);
      else setMappingState(NormalState);
      rawShow();
    }
  }
  ...
  void Client::setMappingState(int s) {
    ...
    // manage() did postpone_geometry_updates = 1, now it's ok to
    // finally set the geometry
    if (was_unmanaged) postponeGeometryUpdates(false);
  }
  ...
  /*!
    ...
    */
  void Client::rawShow() {
    // not really necessary, but let it know the state
    if (decoration != NULL) decoration->widget()->show();
  ...
  */
  ...
  void Client::sendClientMessage(Window w, Atom a, Atom protocol, long data1, long data2, long data3) {
    ...
    mask = 0L;
    if (w == qt_xrootwin()) mask = SubstructureRedirectMask; //HACK: magic!
  ...
  void Client::killWindow() {
    ...
    if (isDialog()) Notify::raise(Notify::TransDelete);
    if (isNormalWindow()) Notify::raise(Notify::Delete);
    killProcess(false);
  ...
  void Client::pingWindow() {
    if (!Pping) return; // can't ping :(
    if (options->killPingTimeout == 0) return; // turned off
    if (ping_timer != NULL) return; // pinging already
  ...
  void Client::gotPing(Time timestamp) {
    ...
    if (NET::timestampCompare(timestamp, ping_timestamp) != 0) return;
    delete ping_timer;
    ping_timer = NULL;
  ...
  void Client::killProcess(bool ask, Time timestamp) {
    ...
    if (!ask) {
      ...
      //TODO: SELI, handle the window created by handler specially (on top,urgent?)
  ...
  void Client::setDesktop(int desktop) {
    ...
    if (decoration != NULL) decoration->desktopChange();
  ...
  void Client::setOnAllDesktops(bool b) {
    if ((b && isOnAllDesktops()) || (!b && !isOnAllDesktops())) return;
    if (b) setDesktop(NET::OnAllDesktops);
    else setDesktop(workspace()->currentDesktop());
  }
  ...
  bool Client::isOnScreen(int screen) const {
    if (!options->xineramaEnabled) return screen == 0;
  ...
  void Client::takeActivity(int flags, bool handled, allowed_t) {
    if (!handled || !Ptakeactivity) {
      if (flags & ActivityFocus) takeFocus(Allowed);
      if (flags & ActivityRaise) workspace()->raiseClient(this);
      return;
    }
  ...
  */
  ...
  void Client::getWMHints() {
    ...
    if (hints)
    {
      if (hints->flags & InputHint) input = hints->input;
      if (hints->flags & WindowGroupHint) window_group = hints->window_group;
  ...
  void Client::getMotifHints() {
    ...
    if (isManaged()) updateDecoration(true);
  }
  ...
  void Client::getIcons() {
    ...
    if (icon_pix.isNull() && isTransient()) {
      ClientList mainclients = mainClients();
      for (cl_iter_c it = mainclients.begin(); it != mainclients.end() && icon_pix.isNull(); ++it) {
        icon_pix = (*it)->icon();
    ...
    if (isManaged() && decoration != NULL) decoration->iconChange();
  }
  ...
  Window Client::staticWmClientLeader(WId w) {
    ...
    if (status == Success) {
      if (data && nitems > 0) result = *((Window*) data);
      XFree(data);
    }
    return result;
  }
  ...
  void Client::getWmClientMachine() {
    ...
    if (client_machine.isEmpty()) client_machine = "localhost";
  }
  ...
  */
  QCString Client::wmClientMachine(bool use_localhost) const {
    ...
    if (use_localhost) {
      if (result != "localhost" && isLocalMachine(result)) result = "localhost";
    }
    return result;
  }
  ...
  */
  Window Client::wmClientLeader() const {
    if (wmClientLeaderWin) return wmClientLeaderWin;
    return window();
  }
...
//TODO: mit nejake checkCursor(), ktere se zavola v manage() a pri vecech, kdy by se kurzor mohl zmenit?
  void Client::setCursor(const QCursor& c) {
    ...
    cursor = c;
    if (decoration != NULL) decoration->widget()->setCursor(cursor);
    XDefineCursor(qt_xdisplay(), frameId(), cursor.handle());
  }

  Client::Position Client::mousePosition(const QPoint& p) const {
    if (decoration != NULL) return decoration->mousePosition(p);
    return PositionCenter;
  }

  void Client::updateAllowedActions(bool force) {
    if (!isManaged() && !force) return;
    unsigned long old_allowed_actions = allowed_actions;
    allowed_actions = 0;
    if (isMovable()) allowed_actions |= NET::ActionMove;
    if (isResizable()) allowed_actions |= NET::ActionResize;
    if (isMinimizable()) allowed_actions |= NET::ActionMinimize;
    if (isShadeable()) allowed_actions |= NET::ActionShade;

    // sticky state not supported
    if (isMaximizable()) allowed_actions |= NET::ActionMax;
    if (userCanSetFullScreen()) allowed_actions |= NET::ActionFullScreen;

    allowed_actions |= NET::ActionChangeDesktop; // always (pagers shouldn't show Docks etc.)

    if (isCloseable()) allowed_actions |= NET::ActionClose;
    if (old_allowed_actions == allowed_actions) return;
  ...
  void Client::setOpacity(bool translucent, uint opacity) {
    // xcompmgr does not like non solid desktops and the user could set
    // it accidently by mouse scrolling
    if (isDesktop()) return;
    ...
    if (!translucent || opacity ==  0xFFFFFFFF) {
      ...
      // ??? frameId() is necessary for visible changes, window()
      // is the winId() that would be set by apps - we set both to be sure
      // the app knows what's currently displayd
      XDeleteProperty(qt_xdisplay(), window(), atoms->net_wm_window_opacity);
    } else {
      if (opacity == opacity_) return;
      opacity_ = opacity;
      long data = opacity; // 32bit XChangeProperty needs long
      XChangeProperty(qt_xdisplay(), frameId(), atoms->net_wm_window_opacity,
                      XA_CARDINAL, 32, PropModeReplace, (uchar *) &data, 1L);
      XChangeProperty(qt_xdisplay(), window(), atoms->net_wm_window_opacity,
                      XA_CARDINAL, 32, PropModeReplace, (uchar *) &data, 1L);
    }
  }
  ...
  void Client::setShadowSize(uint shadowSize) {
    ...
    XChangeProperty(qt_xdisplay(), frameId(), atoms->net_wm_window_shadow,
                    XA_CARDINAL, 32, PropModeReplace, (uchar *) &data, 1L);
  }

  void Client::updateOpacity() {
    // extra syncscreen flag allows to avoid double syncs when active state
    // changes (as it will usually change for two windows)
    if (!(isNormalWindow() || isDialog() || isUtility())|| custom_opacity) return;
    if (isActive()) {
      ...
      if (isBMP()) {
        ...
        while (it != tmpGroupMembers.end()) {
          ...
          if ((*it) != this && (*it)->isBMP()) {
            ...
            // first test, if the new client touches the just activated one
            if ((*it)->touches(this)) {
              ...
              // pot. client does not touch c, so we have to search if it
              // touches some other activated client
              bool found = false;
              for (cl_iter_c it2 = activeGroupMembers.begin(); it2 != activeGroupMembers.end(); it2++) {
      ...
      if (ruleOpacityInactive())
        ...
        setOpacity((options->translucentInactiveWindows &&
          ...
          if ((*it) != this && (*it)->isBMP()) {
            ...
            if ((*it)->touches(this)) {
              ...
              if ((*it)->ruleOpacityInactive())
                ...
                (*it)->setOpacity((options->translucentInactiveWindows &&
                                    !((*it)->keepAbove() && options->keepAboveAsActive)),
                                  options->inactiveWindowOpacity);
              }
  ...
  void Client::updateShadowSize() {
    if (!(isNormalWindow() || isDialog() || isUtility())) return;
    if (isActive()) setShadowSize(options->activeWindowShadowSize);
    else setShadowSize(options->inactiveWindowShadowSize);
  }
  ...
  //query translucency settings from X, returns true if window opacity is set
  bool Client::getWindowOpacity() {
    unsigned char *data = 0;
    Atom actual;
    int format, result;
    unsigned long n, left;
    result = XGetWindowProperty(qt_xdisplay(), window(), atoms->net_wm_window_opacity,
  ...
  bool Client::touches(const Client* c) {
    if (y() == c->y() + c->height()) return TRUE; // this bottom to c
    if (y() + height() == c->y()) return TRUE; // this top to c
    if (x() == c->x() + c->width()) return TRUE; // this right to c
    if (x() + width() == c->x()) return TRUE; // this left to c
    return FALSE;
  }
  ...
  void Client::setDecoHashProperty(uint topHeight, uint rightWidth, uint bottomHeight, uint leftWidth) {
    ...
    XChangeProperty(qt_xdisplay(), frameId(), atoms->net_wm_window_decohash,
                    XA_CARDINAL, 32, PropModeReplace, (uchar *) &data, 1L);
  }
