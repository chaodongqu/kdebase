...
namespace KWinInternal {
  ...
  static int x11ErrorHandler(Display *d, XErrorEvent *e) {
    ...
    if(ignore_badwindow && (e->error_code == BadWindow || e->error_code == BadColor)) { return 0; }
  ...
  , owner(screen_number) {
    ...
    if(screen_number == -1) { screen_number = DefaultScreen(qt_xdisplay()); }
  ...
  Application::~Application() {
    ...
    if(owner.ownerWindow() != None) {
      ...
      DCOPRef ref("kded", "kded");
      if(!ref.send("loadModule", QCString("kdetrayproxy"))) { kdWarning( 176 ) << "Loading of kdetrayproxy failed.\n"; }
    }
    delete options;
  }
  ...
  bool Application::x11EventFilter(XEvent *e) {
    if(Workspace::self()->workspaceEvent(e)) { return TRUE; }
...
extern "C" KDE_EXPORT int kdemain(int argc, char *argv[]) {
  ...
  if (!restored) {
    ...
    if(multiHead.lower() == "true") {
      ...
      // 10 is enough to be sure we removed ".s"
      ...
      if(number_of_screens != 1) {
        for(int i=0; i<number_of_screens; i++) {
          // if execution doesn't pass by here, then kwin acts exactly as previously
          if(i != KWinInternal::screen_number && fork() == 0) {
            KWinInternal::screen_number = i;
            // break here because we are the child process, we don't want to fork() anymore
            break;
          }
        }
  ...
  KAboutData aboutData("kwin", I18N_NOOP("KWin"), version, description, KAboutData::License_GPL,
  ...
  if(signal(SIGTERM, KWinInternal::sighandler) == SIG_IGN) { signal(SIGTERM, SIG_IGN); }
  if(signal(SIGINT, KWinInternal::sighandler) == SIG_IGN) { signal(SIGINT, SIG_IGN); }
  if(signal(SIGHUP, KWinInternal::sighandler) == SIG_IGN) { signal(SIGHUP, SIG_IGN); }
