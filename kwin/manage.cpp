...
namespace KWinInternal {
  ...
  bool Client::manage(Window w, bool isMapped)
  {
    ...
    if (!XGetWindowAttributes(qt_xdisplay(), w, &attr)) return false;
    ...
    if (session) {
      if (session->minimized) init_minimize = true;
      if (session->userNoBorder) setUserNoBorder(true);
    }

    setShortcut(rules()->checkShortcut(session ? session->shortcut : QString::null, true));

    init_minimize = rules()->checkMinimize(init_minimize, !isMapped);
    if (rules()->checkNoBorder(false, !isMapped)) setUserNoBorder(true);
    ...
    if (session) {
      ...
      if (session->onAllDesktops) desk = NET::OnAllDesktops;
    } else {
      ...
      if (isTransient()) {
        ...
        for(ClientList::ConstIterator it = mainclients.begin(); it != mainclients.end(); ++it) {
          // don't consider toolbars etc when placing
          if (mainclients.count() > 1 && (*it)->isSpecialWindow()) continue;

          maincl = *it;
          if ((*it)->isOnCurrentDesktop()) on_current = true;
        }

        if (on_current) desk = workspace()->currentDesktop();
        else if (maincl != NULL) desk = maincl->desktop();
      }

      // window had the initial desktop property, force it
      if (info->desktop()) desk = info->desktop();
    ...
    if (desk == 0) desk = workspace()->currentDesktop();
    ...
    if (session) geom = session->geometry;

    QRect area;
    bool partial_keep_in_area = isMapped || session;
    if (isMapped || session) area = workspace()->clientArea(FullArea, geom.center(), desktop());
    else if (options->xineramaPlacementEnabled) {
      int screen = options->xineramaPlacementScreen;

      // active screen
      if (screen == -1) screen = asn_data.xinerama() == -1 ?
    ...
    if (int type = checkFullScreenHack(geom)) {
      ...
      if (rules()->checkStrictGeometry(false)) {
        // 1 - it's xinerama-aware fullscreen hack, 2 - it's full area
        geom = type == 2 ?
            workspace()->clientArea(FullArea, geom.center(), desktop()) :
            workspace()->clientArea(ScreenArea, geom.center(), desktop());
      } else geom = workspace()->clientArea(FullScreenArea, geom.center(), desktop());
      placementDone = true;
    }
    ...
    if (isMapped || session || placementDone)
      ...
      // if the dialog is actually non-NETWM transient window,
      // don't try to apply placement to it, it breaks with too many
      // things (xmms, display)
      if (mainClients().count() >= 1)
      {
#if 1
        // TODO #78082 - Ok, it seems there are after all some cases when an
        // application has a good reason to specify a position for its dialog.
        // Too bad other WMs have never bothered with placement for dialogs,
        // so apps always specify positions for their dialogs, including such
        // silly positions like always centered on the screen or under mouse.
        // Using ignoring requested position in window-specific settings helps,
        // but at least for Qt apps this should work better.
        usePosition = true;
#else
          ; // force using placement policy
#endif
      } else usePosition = true;
    } else if (isSplash()) ; // force using placement policy
    else usePosition = true;
    ...
    // size is always obeyed for now, only with constraints applied
    if (true)
    ...
    if (xSizeHint.flags & PMaxSize)
      geom.setSize(geom.size().boundedTo(
                   rules()->checkMaxSize(QSize(
                         xSizeHint.max_width, xSizeHint.max_height))));

    if (xSizeHint.flags & PMinSize)
      geom.setSize(geom.size().expandedTo(
                   rules()->checkMinSize(QSize(
                         xSizeHint.min_width, xSizeHint.min_height))));

    if (isMovable()) {
      if (geom.x() > area.right() || geom.y() > area.bottom())
        placementDone = FALSE; //NOTE: weird, do not trust.
    }

    // before gravitating
    if (placementDone) move(geom.x(), geom.y());
    ...
    if (init_minimize && isTransient())
    {
      ClientList mainclients = mainClients();
      for (ClientList::ConstIterator it = mainclients.begin(); it != mainclients.end(); ++it)
        if ((*it)->isShown(true)) init_minimize = false; // SELI even e.g. for NET::Utility?
    }
    ...
    if (!init_minimize && isTransient() && mainClients().count() > 0) {
      ...
      ClientList mainclients = mainClients();
      for (ClientList::ConstIterator it = mainclients.begin(); it != mainclients.end(); ++it)
        if ((*it)->isShown(true)) visible_parent = true;
      if (!visible_parent)
      {
        init_minimize = true;
        demandAttention();
    ...
    if (session) {
      ...
      MaximizeMode maxmode =
          static_cast<MaximizeMode>(((info->state() & NET::MaxVert)?MaximizeVertical:0) |
      ...
      setShade(rules()->checkShade(
               info->state() & NET::Shaded ? ShadeNormal : ShadeNone, !isMapped));
      setKeepAbove(rules()->checkKeepAbove(
                   info->state() & NET::KeepAbove, !isMapped));
      setKeepBelow(rules()->checkKeepBelow(
                   info->state() & NET::KeepBelow, !isMapped));
      setSkipTaskbar(rules()->checkSkipTaskbar(
                     info->state() & NET::SkipTaskbar, !isMapped), true);
      setSkipPager(rules()->checkSkipPager(
                   info->state() & NET::SkipPager, !isMapped));
      if (info->state() & NET::DemandsAttention) demandAttention();
      if (info->state() & NET::Modal) setModal(true);
      if (fullscreen_mode != FullScreenHack && isFullScreenable())
        setFullScreen(rules()->checkFullScreen(
                      info->state() & NET::FullScreen, !isMapped), false);
    }
    ...
    user_time = readUserTimeMapTimestamp(asn_valid ? &asn_id : NULL,
                                         asn_valid ? &asn_data : NULL, session);
    group()->updateUserTime(user_time);   // and do what Client::updateUserTime() does

    // they're shown in Workspace::addClient() if their mainwindow
    if (isTopMenu()) hideClient(true); // is the active one
    ...
    if (isShown(true) && !doNotShow) {
      ...
      if (!isMapped)
      {
        if (allow && isOnCurrentDesktop()) {
          ...
          if (!session && !isSpecialWindow()) demandAttention();
        }
      }
