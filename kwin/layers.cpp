...
//*******************************
  void Workspace::updateClientLayer(Client* c) {
    if (c == NULL) return;
    if (c->layer() == c->belongsToLayer()) return;
    StackingUpdatesBlocker blocker(this);
    c->invalidateLayer(); // invalidate, will be updated when doing restacking
    for (cl_iter_c it = c->transients().begin(); it != c->transients().end(); ++it)
  ...
  void Workspace::updateStackingOrder(bool propagate_new_clients) {
    ...
    if (changed || propagate_new_clients) {
      ...
      if (active_client) active_client->updateMouseGrab();
    }
  }
  ...
  */
  void Workspace::propagateClients(bool propagate_new_clients) {
    ...
    for (cl_iter_c it = stacking_order.fromLast(); it != stacking_order.end(); --it) {
      ...
      if ((*it)->belongsToLayer() >= DockLayer) topmenu_space_pos = pos;
    }

    if (topmenu_space != NULL) {
      // make sure the topmenu space is below all topmenus, fullscreens, etc.
      for (int i = pos; i > topmenu_space_pos; --i)
    ...
    if (propagate_new_clients) {
      ...
      for (cl_iter_c it = desktops.begin(); it != desktops.end(); ++it)
        cl[pos++] = (*it)->window();
  ...
  */
  ...
  Client* Workspace::topClientOnDesktop(int desktop, bool unconstrained, bool only_normal) const {
    ...
    for (cl_iter_c it = begin; it != end; --it) {
      if ((*it)->isOnDesktop(desktop) && (*it)->isShown(false)) {
        if (!only_normal) return *it;
        if ((*it)->wantsTabFocus() && !(*it)->isSpecialWindow()) return *it;
      }
    }
    return 0;
  }
  ...
  void Workspace::raiseOrLowerClient(Client *c) {
    ...
    if (c == topmost) lowerClient(c);
    else raiseClient(c);
  }

  void Workspace::lowerClient(Client* c) {
    if (!c) return;
    if (c->isTopMenu()) return;
    ...
    if (c->isTransient()) {
      ...
      for (cl_iter_c it = mainclients.fromLast(); it != mainclients.end(); ++it)
        lowerClient(*it);
    }
  ...
  void Workspace::lowerClientWithinApplication(Client* c) {
    ...
    for (cl_iter it = unconstrained_stacking_order.begin(); it != unconstrained_stacking_order.end(); ++it)
      if (Client::belongToSameApplication(*it, c)) {
        unconstrained_stacking_order.insert(it, c);
        lowered = true;
        break;
      }
    if (!lowered) unconstrained_stacking_order.prepend(c);
    // ignore mainwindows
  }
  ...
  void Workspace::raiseClientWithinApplication(Client* c) {
    ...
    for (cl_iter it = unconstrained_stacking_order.fromLast(); it != unconstrained_stacking_order.end(); --it) {
  ...
  void Workspace::lowerClientRequest(Client* c, NET::RequestSource src, Time /*timestamp*/) {
    ...
    if (src == NET::FromTool || !c->hasUserTimeSupport()) lowerClient(c);
    else lowerClientWithinApplication(c);
  }
  ...
  void Workspace::restackClientUnderActive(Client* c) {
    ...
    for (int desktop = 1; desktop <= numberOfDesktops(); ++desktop) {
      ...
      if (c->wantsTabFocus() && c->isOnDesktop(desktop) && focus_chain[ desktop ].contains(active_client))
      {
        if (Client::belongToSameApplication(active_client, c)) {
          ...
          // belonging to the active applicationa
          focus_chain[desktop].remove(c);
          for (cl_iter it = focus_chain[desktop].fromLast(); it != focus_chain[desktop].end(); --it) {
            if (Client::belongToSameApplication(active_client, *it)) {
    ...
    if (c->wantsTabFocus() && global_focus_chain.contains(active_client)) {
      if (Client::belongToSameApplication(active_client, c)) {
        ...
        global_focus_chain.remove(c);
        for (cl_iter it = global_focus_chain.fromLast(); it != global_focus_chain.end(); --it) {
          if (Client::belongToSameApplication(active_client, *it)) {
  ...
  void Workspace::circulateDesktopApplications() {
    if (desktops.count() > 1) {
      ...
      if (change_active) activateClient(findDesktop(true, currentDesktop()));
    }
  ...
  */
  ClientList Workspace::constrainedStackingOrder() {
    ...
    for (cl_iter_c it = unconstrained_stacking_order.begin(); it != unconstrained_stacking_order.end(); ++it) {
      Layer l = (*it)->layer();
    ...
    ClientList stacking;
    for (Layer lay = FirstLayer; lay < NumLayers; ++lay) stacking += layer[lay];
#if 0
    kdDebug() << "stacking2:" << endl;
    for (cl_iter_c it = stacking.begin(); it != stacking.end(); ++it)
      kdDebug() << (void*)(*it) << *it << ":" << (*it)->layer() << endl;
#endif
    // now keep transients above their mainwindows
    //TODO: this could(?) use some optimization
    for (cl_iter it = stacking.fromLast(); it != stacking.end(); ) {
      if (!(*it)->isTransient()) {
        --it;
        continue;
      }
      ...
      if ((*it)->groupTransient()) {
        if ((*it)->group()->members().count() > 0) {
          ...
          for (it2 = stacking.fromLast(); it2 != stacking.end(); --it2) {
            if (*it2 == *it) {
...
//*******************************
  ...
  void Client::restackWindow(Window /*above*/, //TODO: above part
                             int detail,
    ...
    if (send_event) sendSyntheticConfigureNotify();
  }

  void Client::setKeepAbove(bool b) {
    b = rules()->checkKeepAbove(b);
    if (b && !rules()->checkKeepBelow(false)) setKeepBelow(false);
    if (b == keepAbove()) {
    ...
    keep_above = b;
    info->setState(keepAbove() ? NET::KeepAbove : 0, NET::KeepAbove);
    if (decoration != NULL) decoration->emitKeepAboveChanged(keepAbove());
  ...
  void Client::setKeepBelow(bool b) {
    ...
    if (b && !rules()->checkKeepAbove(false)) setKeepAbove(false);
    if (b == keepBelow()) {
      // force hint change if different
    ...
    keep_below = b;
    info->setState(keepBelow() ? NET::KeepBelow : 0, NET::KeepBelow);
    if (decoration != NULL) decoration->emitKeepBelowChanged(keepBelow());
    workspace()->updateClientLayer(this);
    updateWindowRules();
  }
  ...
  {
    if (isDesktop()) return DesktopLayer;

    // no annoying splashscreens getting in the way of everything else
    if (isSplash()) return NormalLayer;
    // slight hack for the 'allow window to cover panel' Kicker setting
    // don't move keepbelow docks below normal window, but only to the same
    // layer, so that both may be raised to cover the other
    if (isDock() && keepBelow()) return NormalLayer;
    if (keepBelow()) return BelowLayer;
    if (isDock() && !keepBelow()) return DockLayer;
    if (isTopMenu()) return DockLayer;
