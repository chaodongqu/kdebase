...
namespace KWinInternal
{
  ...
  */
  void Placement::place(Client* c, QRect& area) {
    ...
    if (c->isUtility()) placeUtility(c, area, options->placement);
    else if (c->isDialog()) placeDialog(c, area, options->placement);
    // on mainwindow, if any, otherwise centered
    else if (c->isSplash()) placeOnMainWindow(c, area);
    else place(c, area, options->placement);
  }

  void Placement::place(Client* c, QRect& area, Policy policy, Policy nextPlacement) {
    if (policy == Unknown) policy = Default;
    if (policy == Default) policy = options->placement;
    if (policy == NoPlacement) return;
    else if (policy == Random) placeAtRandom(c, area, nextPlacement);
    else if (policy == Cascade) placeCascaded(c, area, nextPlacement);
    else if (policy == Centered) placeCentered(c, area, nextPlacement);
    else if (policy == ZeroCornered) placeZeroCornered(c, area, nextPlacement);
  ...
  */
  void Placement::placeAtRandom(Client* c, const QRect& area, Policy /*next*/)
  {
    ...
    if (px < maxRect.x()) px = maxRect.x();
    if (py < maxRect.y()) py = maxRect.y();

    px += step;
    py += 2*step;

    if (px > maxRect.width()/2) px =  maxRect.x() + step;
    if (py > maxRect.height()/2) py =  maxRect.y() + step;
  ...
  */
  void Placement::placeSmart(Client* c, const QRect& area, Policy /*next*/) {
    ...
    //BEGIN: loop over possible positions
    do {
    ...
    //END: loop over possible positions

    if (ch>= maxRect.height()) y_optimal=maxRect.top();
  ...
  */
  ...
  void Placement::placeOnMainWindow(Client* c, QRect& area, Policy nextPlacement) {
    if (nextPlacement == Unknown) nextPlacement = Centered;

    // maximize if needed
    if (nextPlacement == Maximizing) placeMaximizing(c, area, NoPlacement);
    ...
    int mains_count = 0;
    for (ClientList::ConstIterator it = mainwindows.begin(); it != mainwindows.end(); ++it) {
      // don't consider toolbars etc when placing
      if (mainwindows.count() > 1 && (*it)->isSpecialWindow()) continue;

      ++mains_count;
      place_on2 = *it;
      if ((*it)->isOnCurrentDesktop()) {
        if (place_on == NULL) place_on = *it;
        else {
  ...
  Placement::Policy Placement::policyFromString(const QString& policy, bool no_special) {
    if (policy == "NoPlacement") return NoPlacement;
    else if (policy == "Default" && !no_special) return Default;
    else if (policy == "Random") return Random;
    else if (policy == "Cascade") return Cascade;
    else if (policy == "Centered") return Centered;
...
// ********************
  ...
  */
  ...
  void Workspace::slotWindowShrinkHorizontal() {
    if (active_client) active_client->shrinkHorizontal();
  }
  ...
  void Client::shrinkHorizontal() {
    ...
    if (geom.width() <= 1) return;
    geom.setSize(adjustedSize(geom.size(), SizemodeFixedW));
    if (geom.width() > 20) setGeometry(geom);
  }

  void Workspace::slotWindowGrowVertical() {
    if (active_client) active_client->growVertical();
  }
  ...
  void Client::shrinkVertical() {
    ...
    if (geom.height() <= 1) return;
    geom.setSize(adjustedSize(geom.size(), SizemodeFixedH));
    if (geom.height() > 20) setGeometry(geom);
  }
  ...
  int Workspace::packPositionLeft(const Client* cl, int oldx, bool left_edge) const {
    ...
    if (oldx <= newx) return oldx;
    for (ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it) {
      if (!(*it)->isShown(false) || !(*it)->isOnDesktop(active_client->desktop())) continue;
  ...
  int Workspace::packPositionRight(const Client* cl, int oldx, bool right_edge) const {
    ...
    if (oldx >= newx) return oldx;
    for (ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it) {
      if (!(*it)->isShown(false) || !(*it)->isOnDesktop(cl->desktop())) continue;
