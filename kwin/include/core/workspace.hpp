...
namespace KWinInternal {
  ...
  public:
    ...
    bool operator==(const SystemTrayWindow& other) { return win == other.win; }
    WId win;
    WId winFor;
  };
  ...
  public slots:
    ...
    void slotPlaceWindowLeft(); /// Place active window to the left half of active screen
    void slotPlaceWindowRight(); /// Place active window to the right half of active screen
    void slotPlaceWindowTop(); /// Place acive window to the top half of active screen
    void slotPlaceWindowBottom(); /// Plave active window to the bottom half of active screen
  ...
  private:
    ...
    unsigned int sendFakedMouseEvent(QPoint pos, WId win, MouseEmulation type, int button, unsigned int state);
  ...
  */
  ...
  {
  public:
    StackingUpdatesBlocker(Workspace* w) : ws(w) { ws->blockStackingUpdates(true); }
    ~StackingUpdatesBlocker() { ws->blockStackingUpdates(false); }
  private:
    Workspace* ws;
  };
  ...
  inline Client* Workspace::findClient(T predicate) {
    if(Client* ret = findClientInList(clients, predicate)) return ret;
    if(Client* ret = findClientInList(desktops, predicate)) return ret;
    return NULL;
  }
  ...
  inline void Workspace::forEachClient(T1 procedure, T2 predicate) {
    for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it)
      if(predicate(const_cast< const Client* >(*it))) procedure(*it);
