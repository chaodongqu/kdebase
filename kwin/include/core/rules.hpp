...
namespace KWinInternal {
  ...
  class WindowRules : public KDecorationDefines {
    public:
  ...
  class Rules : public KDecorationDefines {
    public:
      Rules();
      Rules( KConfig& );
  ...
  inline bool Rules::checkSetRule(SetRule rule, bool init) {
    //NOTE: Unused or DontAffect
    if(rule > (SetRule)DontAffect) {
      if(rule == (SetRule)Force || rule == (SetRule)ApplyNow || rule == (SetRule)ForceTemporarily || init) return true;
    }
    return false;
  }
  ...
  inline bool Rules::checkSetStop(SetRule rule) { return rule != UnusedSetRule; }

  inline bool Rules::checkForceStop(ForceRule rule) { return rule != UnusedForceRule; }

  inline WindowRules::WindowRules(const QValueVector< Rules* >& r) : rules(r) { }
  ...
  inline void WindowRules::remove(Rules* rule) {
    ...
    if(pos != rules.end()) { rules.erase(pos); }
  }

#endif

#ifdef NDEBUG
inline kndbgstream& operator<<(kndbgstream& stream, const Rules*) { return stream; }
#else
#ifndef KCMRULES
