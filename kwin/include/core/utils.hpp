...
namespace KWinInternal {
  ...
  //TODO: fix to long names (probably will take year or so) but for now typedef is "fix"
  ...
  inline void operator++(Layer& lay) { lay = static_cast< Layer >(lay + 1); }
...
// compile with XShape older than 1.0
  ...
  template< typename T > class TemporaryAssign {
  public:
    TemporaryAssign(const T& var, const T& value) : variable(var), orig(var) {
...
//NOTE: <type_helper> - in order to work also with type being 'const Client*' etc.
#define KWIN_COMPARE_PREDICATE( name, type, check ) \
  struct name { \
  typedef type type_helper; \
...
#define KWIN_PROCEDURE( name, action ) \
  ...
  template< typename T > Client* findClientInList(const ClientList& list, T predicate) {
    for (ClientList::ConstIterator it = list.begin(); it != list.end(); ++it) {
      if (predicate(const_cast< const Client* >(*it))) return *it;
    }
    return NULL;
  }

  // like strcmp()
  inline int timestampCompare(Time time1, Time time2) { return NET::timestampCompare(time1, time2); }
  // returns time2 - time1
  inline Time timestampDiff(Time time1, Time time2) { return NET::timestampDiff(time1, time2); }
