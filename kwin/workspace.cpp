...
namespace KWinInternal
{
  ...
  forced_global_mouse_grab(false)
  {
    ...
    XChangeProperty(qt_xdisplay(), qt_xrootwin(), atoms->kwin_running,
                    atoms->kwin_running, 32, PropModeAppend, (unsigned char*) &data, 1);
  ...
  void Workspace::init() {
    ...
    int initial_desktop;
    if(!kapp->isSessionRestored()) initial_desktop = client_info.currentDesktop();
    else {
      KConfigGroupSaver saver(kapp->sessionConfig(), "Session");
      initial_desktop = kapp->sessionConfig()->readNumEntry("desktop", 1);
    }
    if(!setCurrentDesktop(initial_desktop)) setCurrentDesktop(1);

    // now we know how many desktops we'll have, thus, we initialise the positioning object
    initPositioning = new Placement(this);

    connect(&reconfigureTimer, SIGNAL(timeout()), this, SLOT(slotReconfigure()));
    connect(&updateToolWindowsTimer, SIGNAL(timeout()), this, SLOT(slotUpdateToolWindows()));
    connect(kapp, SIGNAL(appearanceChanged()), this, SLOT(slotReconfigure()));
    connect(kapp, SIGNAL(settingsChanged(int)), this, SLOT(slotSettingsChanged(int)));
    ...
    if(!kapp->isSessionRestored()) ++block_focus;
    ...
    //BEGIN: updates blocker block
    {
      ...
      for(i = 0; i < nwins; i++) {
        ...
        if(topmenu_space && topmenu_space->winId() == wins[i]) continue;
        if(attr.map_state != IsUnmapped) {
          if(addSystemTrayWin(wins[i])) continue;
    ...
    if(new_active_client == NULL && activeClient() == NULL && should_get_focus.count() == 0) {
    ...
    if(new_active_client != NULL) activateClient(new_active_client);

    //TODO: SELI, this won't work with unreasonable focus policies,
  ...
  void Workspace::addClient(Client* c, allowed_t) {
    ...
    if(c->isDock()) {
      ...
      // this should be done slightly more efficient,
      // but we want to support the topDock in future
      if(!c->hasCustomOpacity()) {
    ...
    if(c->isDesktop()) {
      ...
      if(active_client == NULL && should_get_focus.isEmpty() && c->isOnCurrentDesktop())
        //TODO: CHECKME? make sure desktop is active after startup if there's no other window active
        requestFocus(c);
    } else {
      updateFocusChains(c, FocusChainUpdate);   // add to focus chain if not already there
      clients.append(c);
    }
    if(!unconstrained_stacking_order.contains(c)) unconstrained_stacking_order.append(c);

    // it'll be updated later, and updateToolWindows() requires
    // c to be in stacking_order
    if(!stacking_order.contains(c)) stacking_order.append(c);

    if(c->isTopMenu()) addTopMenu(c);
    ...
    if(c->isUtility() || c->isMenu() || c->isToolbar()) updateToolWindows(true);
  ...
  */
  void Workspace::removeClient(Client* c, allowed_t) {
    if(c == active_popup_client) closeActivePopup();

    if(client_keys_client == c) setupWindowShortcutDone(false);
    // remove from client_keys
    if(!c->shortcut().isNull()) c->setShortcut(QString::null);

    if(c->isDialog()) Notify::raise(Notify::TransDelete);
    if(c->isNormalWindow()) Notify::raise(Notify::Delete);
    ...
    for(int i = 1; i <= numberOfDesktops(); ++i) focus_chain[ i ].remove(c);
    global_focus_chain.remove(c);
    attention_chain.remove(c);
    showing_desktop_clients.remove(c);
    if(c->isTopMenu()) removeTopMenu(c);
    Group* group = findGroup(c->window());
    if(group != NULL) group->lostLeader();

    if(c == most_recently_raised) most_recently_raised = 0;
    should_get_focus.remove(c);
    Q_ASSERT(c != active_client);
    if(c == last_active_client) last_active_client = 0;
    if(c == pending_take_activity) pending_take_activity = NULL;
    if(c == delayfocus_client) cancelDelayFocus();

    updateStackingOrder(true);

    if(tab_grab) tab_box->repaint();
    ...
    if(c->desktop() == NET::OnAllDesktops) {
      ...
      for(int i=1; i<= numberOfDesktops(); i++) {
        ...
        if(i == currentDesktop()
          ...
          if(change == FocusChainMakeFirst) focus_chain[ i ].append(c);
          else focus_chain[ i ].prepend(c);
  ...
  void Workspace::updateCurrentTopMenu() {
    ...
    if(active_client) {
      ...
      for(;;) {
        ...
        for(cl_iter_c it = menu_client->transients().begin(); it != menu_client->transients().end(); ++it)
          if((*it)->isTopMenu()) {
            menubar = *it;
            break;
          }
      ...
      if(!menubar) {
        ...
        for(cl_iter_c it = active_client->group()->members().begin(); it != active_client->group()->members().end(); ++it)
          if((*it)->isTopMenu()) {
            menubar = *it;
            break;
          }
    ...
    if(!menubar && !block_desktop_menubar && options->desktopTopMenu()) {
      ...
      if(desktop != NULL) {
        for(cl_iter_c it = desktop->transients().begin(); it != desktop->transients().end(); ++it)
          if((*it)->isTopMenu()) {
            menubar = *it;
            break;
          }
      ...
      if(menubar == NULL) {
        for(cl_iter_c it = topmenus.begin(); it != topmenus.end(); ++it)
          // kdesktop's topmenu has WM_TRANSIENT_FOR set pointing to the root window
          // to recognize it here. Also, with the xroot hack in kdesktop,
    ...
    for(cl_iter_c it = clients.begin(); it != clients.end(); ++it)
    {
    ...
    for(cl_iter_c it = stacking_order.begin(); it != stacking_order.end(); ++it) {
      if((*it)->isUtility() || (*it)->isMenu() || (*it)->isToolbar()) {
        ...
        if(!show && also_hide) {
          ...
          if(mainclients.isEmpty()) show = true;
          for(cl_iter_c it2 = mainclients.begin(); it2 != mainclients.end(); ++it2) {
            if((*it2)->isSpecialWindow()) show = true;
          }
          if(!show) to_hide.append(*it);
        }

        if(show) to_show.append(*it);
      }
    } // first show new ones, then hide

    // from topmost
    //TODO: since this is in stacking order, the order of taskbar entries changes :(
    for(cl_iter_c it = to_show.fromLast(); it != to_show.end(); --it)
      (*it)->hideClient(false);

    if(also_hide) {
      // from bottommost
      for(cl_iter_c it = to_hide.begin(); it != to_hide.end(); ++it)
        (*it)->hideClient(true);
  ...
  */
  ...
  void Workspace::slotSettingsChanged(int category) {
    ...
    if(category == (int) KApplication::SETTINGS_SHORTCUTS) readShortcuts();
  }
  ...
  */
  ...
  void Workspace::slotReconfigure() {
    ...
    if(options->topMenuEnabled() && !managingTopMenus()) {
      if(topmenu_selection->claim(false)) setupTopMenuHandling();
      else lostTopMenuSelection();
    ...
    loadWindowRules();
    for(cl_iter it = clients.begin(); it != clients.end(); ++it) {
    ...
    // need restart
    if(options->resetKompmgr) {
      bool tmp = options->useTranslucency;
      stopKompmgr();
      // wait some time to ensure system's ready for restart
      if(tmp) QTimer::singleShot(200, this, SLOT(startKompmgr()));
    }
  }
  ...
  void Workspace::loadDesktopSettings() {
    ...
    QCString groupname;
    if(screen_number == 0) groupname = "Desktops";
    else groupname.sprintf("Desktops-screen-%d", screen_number);
    ...
    for(int i = 1; i <= n; i++) {
      QString s = c->readEntry(QString("Name_%1").arg(i), i18n("Desktop %1").arg(i));
  ...
  void Workspace::saveDesktopSettings() {
    ...
    for(int i = 1; i <= number_of_desktops; i++) {
      ...
      if(s != defaultvalue) {
        ...
        if(currentvalue != defaultvalue) c->writeEntry(QString("Name_%1").arg(i), "");
      }
    }
  }
  ...
  */
  ...
  void ObscuringWindows::create(Client* c) {
    if(cached == 0) cached = new QValueList<Window>;
    Window obs_win;
  ...
  ObscuringWindows::~ObscuringWindows() {
    ...
    for(QValueList<Window>::ConstIterator it = obscuring_windows.begin(); it != obscuring_windows.end(); ++it) {
      XUnmapWindow(qt_xdisplay(), *it);
      if(cached->count() < max_cache_size) cached->prepend(*it);
      else XDestroyWindow(qt_xdisplay(), *it);
    }
  }
  ...
  */
  bool Workspace::setCurrentDesktop(int new_desktop) {
    if(new_desktop < 1 || new_desktop > number_of_desktops) return false;
      ...
      */
      ...
      for(cl_iter_c it = stacking_order.begin(); it != stacking_order.end(); ++it)
        if(!(*it)->isOnDesktop(new_desktop) && (*it) != movingClient) {
          if((*it)->isShown(true) && (*it)->isOnDesktop(old_desktop)) obs_wins.create(*it);
          (*it)->updateVisibility();
        }
      ...
      for(cl_iter_c it = stacking_order.fromLast(); it != stacking_order.end(); --it)
        if((*it)->isOnDesktop(new_desktop)) (*it)->updateVisibility();

      --block_showing_desktop;

      // do this only after desktop change to avoid flicker
      if(showingDesktop()) resetShowingDesktop(false);
    }
    ...
    if(options->focusPolicyIsReasonable()) {
      ...
      if(!c) {
        for(cl_iter_c it = focus_chain[currentDesktop()].fromLast(); it != focus_chain[currentDesktop()].end(); --it) {
    ...
    if(c == NULL && !desktops.isEmpty()) c = findDesktop(true, currentDesktop());
  ...
  int Workspace::desktopToRight(int desktop) const {
    ...
    if(layoutOrientation == Qt::Vertical) {
      ...
      if(dt >= numberOfDesktops()) {
        if(options->rollOverDesktops) dt -= numberOfDesktops();
        else return desktop;
      }
    } else {
      int d = (dt % x) + 1;
      if(d >= x) {
        if(options->rollOverDesktops) d -= x;
        else return desktop;
      }
      dt = dt - (dt % x) + d;
    }
  ...
  int Workspace::desktopUp(int desktop) const {
    ...
    if(layoutOrientation == Qt::Horizontal) {
      ...
      int d = (dt % y) - 1;
      if(d < 0) {
        if(options->rollOverDesktops) d += y;
        else return desktop;
      }
      dt = dt - (dt % y) + d;
    }
  ...
  */
  void Workspace::setNumberOfDesktops(int n) {
    if(n == number_of_desktops) return;
    int old_number_of_desktops = number_of_desktops;
    number_of_desktops = n;

    if(currentDesktop() > numberOfDesktops()) setCurrentDesktop(numberOfDesktops());
  ...
  */
  ...
  int Workspace::activeScreen() const {
    if(!options->xineramaEnabled) return 1;
  ...
  QRect Workspace::sc_geom(int screen) const {
    if(!options->xineramaEnabled) return qApp->desktop()->geometry();
  ...
  void Workspace::sendClientToScreen(Client* c, int screen) {
    //NOTE: don't use isOnScreen(), that's true even when only partially
    if(c->screen() == screen) return;
    ...
    if(c->isActive()) active_screen = screen;
  }

  //NOTE: DCOP-only, unused
  ...
  void Workspace::updateDesktopLayout() {
    ...
    if(layoutX == 0 && layoutY == 0) layoutY = 2;
  }
  ...
  void Workspace::calcDesktopLayout(int &x, int &y) const {
    ...
    y = layoutY;
    if((x <= 0) && (y > 0)) x = (numberOfDesktops()+y-1) / y;
    else if((y <=0) && (x > 0)) y = (numberOfDesktops()+x-1) / x;
  ...
  */
  bool Workspace::removeSystemTrayWin(WId w, bool check) {
    if(!systemTrayWins.contains(w)) return FALSE;
    if(check) {
  ...
  */
  ...
  void Workspace::killWindowId(Window window_to_kill) {
    ...
    if(client != NULL) client->killWindow();
    else XKillClient(qt_xdisplay(), window_to_kill);
  }
  ...
  */
  WId Workspace::getMouseEmulationWindow() {
    ...
    do {
      w = child;
      if(!c) c = findClient(FrameIdMatchPredicate(w));
      XQueryPointer(qt_xdisplay(), w, &root, &child,
                    &root_x, &root_y, &lx, &ly, &state);
    } while (child != None && child != w);

    if(c && !c->isActive()) activateClient(c);
    return (WId) w;
  }
  ...
  */
  bool Workspace::keyPressMouseEmulation(XKeyEvent& ev) {
    if(root != qt_xrootwin()) return FALSE;
    ...
    switch (kc) {
      ...
      case XK_KP_Down:
        ...
        break;
      case XK_F1:
        if(!mouse_emulation_state) mouse_emulation_window = getMouseEmulationWindow();
  ...
  */
  ...
  void Workspace::checkElectricBorders(bool force) {
    ...
    if(options->electricBorders() == Options::ElectricAlways) createBorderWindows();
    else destroyBorderWindows();
  }

  void Workspace::createBorderWindows() {
    if(electric_have_borders) return;
  ...
  */
  ...
  void Workspace::clientMoved(const QPoint &pos, Time now) {
    if(options->electricBorders() == Options::ElectricDisabled) return;
    ...
    int border = 0;
    if(pos.x() == electricLeft) border = 1;
    else if(pos.x() == electricRight) border = 2;
    else if(pos.y() == electricTop) border = 3;
    else if(pos.y() == electricBottom) border = 4;
  ...
  */
  bool Workspace::electricBorder(XEvent *e) {
    if(!electric_have_borders) return false;
  ...
  */
  ...
  void Workspace::lostTopMenuOwner() {
    if(!options->topMenuEnabled()) return;
  ...
  */
  ...
  void Workspace::helperDialog(const QString& message, const Client* c) {
    ...
    if(!type.isEmpty()) {
      ...
      // has don't show again checked
      if(!cfg.readBoolEntry(type, true)) return;
      // save launching kdialog
      proc << "--dontagain" << "kwin_dialogsrc:" + type;
    }
    if(c != NULL) proc << "--embed" << QString::number(c->window());
    proc.start(KProcess::DontCare);
  }

  // kompmgr stuff
  void Workspace::startKompmgr() {
    if(!kompmgr || kompmgr->isRunning()) return;
  ...
  */
  ...
  void Workspace::restartKompmgr() {
    // this is for inernal purpose (crashhandling) only,
    // usually you want to use workspace->stopKompmgr();
    // QTimer::singleShot(200, workspace, SLOT(startKompmgr()));

    // uh-ohh
    if(!allowKompmgrRestart) {
      delete kompmgr_selection;
      kompmgr_selection = NULL;
      options->useTranslucency = FALSE;
      KProcess proc;
  ...
  void Workspace::handleKompmgrOutput(KProcess* , char *buffer, int buflen) {
    ...
    if(output.contains("Started",false)) ; // don't do anything, just pass to the connection release
    else if(output.contains("Can't open display",false))
      message = i18n("<qt><b>kompmgr failed to open the display</b><br>"
                     "There is probably an invalid display entry in your"
                     " ~/.xcompmgrrc.</qt>");
    else if(output.contains("No render extension",false))
      message = i18n("<qt><b>kompmgr cannot find the Xrender extension</b>"
                     "<br>You are using either an outdated or a crippled "
                     "version of XOrg.<br>Get XOrg &ge; 6.8 from "
                     "www.freedesktop.org.<br></qt>");
    else if(output.contains("No composite extension",false))
      message = i18n("<qt><b>Composite extension not found</b><br>You <i>must"
                     "</i> use XOrg &ge; 6.8 for translucency and shadows to"
                     " work.<br>Additionally, you need to add a new section "
                     "to your X config file:<br><i>Section \"Extensions\"<br>"
                     "Option \"Composite\" \"Enable\"<br>"
                     "EndSection</i></qt>");
    else if(output.contains("No damage extension",false))
      message = i18n("<qt><b>Damage extension not found</b><br>You <i>must"
                     "</i> use XOrg &ge; 6.8 for translucency and shadows "
                     "to work.</qt>");
    else if(output.contains("No XFixes extension",false))
      message = i18n("<qt><b>XFixes extension not found</b><br>You <i>must</i>"
                     " use XOrg &ge; 6.8 for translucency and shadows to work."
                     "</qt>");
  ...
  void Workspace::setShowingDesktop(bool showing) {
    ...
    if(showing_desktop) {
      ...
      for(cl_iter_c it = cls.begin(); it != cls.end(); ++it) {
      ...
      for(cl_iter_c it = showing_desktop_clients.begin(); it != showing_desktop_clients.end(); ++it)
        (*it)->minimize(true);
      --block_focus;
      if(Client* desk = findDesktop(true, currentDesktop())) requestFocus(desk);
    } else {
      for(cl_iter_c it = showing_desktop_clients.begin(); it != showing_desktop_clients.end(); ++it)
        (*it)->unminimize(true);
      if(showing_desktop_clients.count() > 0) requestFocus(showing_desktop_clients.first());
      showing_desktop_clients.clear();
    }
    --block_showing_desktop;
  }
  ...
  */
  void Workspace::resetShowingDesktop(bool keep_hidden) {
    if(block_showing_desktop > 0) return;
  ...
  */
  void Workspace::slotDisableGlobalShortcuts() {
    if(global_shortcuts_disabled || global_shortcuts_disabled_for_client) disableGlobalShortcuts(false);
    else disableGlobalShortcuts(true);
  }

  static bool pending_dfc = false;

  void Workspace::disableGlobalShortcutsForClient(bool disable) {
    if(global_shortcuts_disabled_for_client == disable) return;
    if(!global_shortcuts_disabled) {
      if(disable) pending_dfc = true;
      KIPC::sendMessageAll(KIPC::BlockShortcuts, disable);
      //NOTE: kwin will get the kipc message too
    }
  }
  ...
  void Workspace::kipcMessage(int id, int data) {
    if(id != KIPC::BlockShortcuts) return;
    if(pending_dfc && data) {
