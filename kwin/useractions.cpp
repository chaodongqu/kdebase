...
namespace KWinInternal {
  ...
  QPopupMenu* Workspace::clientPopup() {
    if(!popup) {
      ...
      advanced_popup->insertItem(SmallIconSet("up"),
                                 i18n("Keep &Above Others")+'\t'+keys->shortcut("Window Above Other Windows").seq(0).toString(),
                                 Options::KeepAboveOp);
      advanced_popup->insertItem(SmallIconSet("down"),
                                 i18n("Keep &Below Others")+'\t'+keys->shortcut("Window Below Other Windows").seq(0).toString(),
                                 Options::KeepBelowOp);
      advanced_popup->insertItem(SmallIconSet("window_fullscreen"),
                                 i18n("&Fullscreen")+'\t'+keys->shortcut("Window Fullscreen").seq(0).toString(),
                                 Options::FullScreenOp);
      advanced_popup->insertItem(i18n("&No Border")+'\t'+keys->shortcut("Window No Border").seq(0).toString(),
                                 Options::NoBorderOp);
      advanced_popup->insertItem(SmallIconSet("key_bindings"),
                                 i18n("Window &Shortcut...")+'\t'+keys->shortcut("Setup Window Shortcut").seq(0).toString(),
      ...
      popup->insertItem(SmallIconSet("fileclose"), i18n("&Close")+'\t'+keys->shortcut("Window Close").seq(0).toString(),
                        Options::CloseOp);
    }
    return popup;
  }
  ...
  */
  ...
  {
    if(!active_popup_client || !popup) return;
  ...
  */
  void Workspace::desktopPopupAboutToShow() {
    ...
    if(active_popup_client && active_popup_client->isOnAllDesktops()) desk_popup->setItemChecked(0, TRUE);
    ...
    for(int i = 1; i <= numberOfDesktops(); i++) {
      ...
      id = desk_popup->insertItem(basic_name.arg(i).arg(desktopName(i).replace('&', "&&")), i);
      if(active_popup_client && !active_popup_client->isOnAllDesktops() && active_popup_client->desktop()  == i)
...
#include "kwinbindings.cpp"
  ...
  void Workspace::setupWindowShortcut(Client* c) {
    ...
    if(pos.x() + size.width() >= r.right()) pos.setX(r.right() - size.width());
    if(pos.y() + size.height() >= r.bottom()) pos.setY(r.bottom() - size.height());
  ...
  void Workspace::clientPopupActivated(int id) {
    ...
    switch(op) {
      case FullScreenOp:
        if(!c->isFullScreen() && c->userCanSetFullScreen()) type = "fullscreenaltf3";
        break;
      case NoBorderOp:
        if(!c->noBorder() && c->userCanSetNoBorder()) type = "noborderaltf3";
        break;
      default:
        break;
    };
    if(!type.isEmpty()) helperDialog(type, c);
  ...
  void Workspace::performWindowOperation(Client* c, Options::WindowOperation op) {
    ...
    switch (op) {
      ...
      case Options::MaximizeOp:
        c->maximize(c->maximizeMode() == Client::MaximizeFull ? Client::MaximizeRestore : Client::MaximizeFull);
        break;
      ...
      case Options::KeepAboveOp: {
        ...
        if (was && !c->keepAbove()) raiseClient(c);
        break;
      }
      case Options::KeepBelowOp: {
        ...
        if (was && !c->keepBelow()) lowerClient(c);
        break;
      }
  ...
  */
  bool Client::performMouseCommand(Options::MouseCommand command, QPoint globalPos, bool handled) {
    ...
    switch (command) {
      ...
      case Options::MouseOperationsMenu:
        if(isActive() && options->clickRaise) autoRaise();
      ...
      case Options::MouseUnrestrictedMove: {
        if(!isMovable()) break;
        if(moveResizeMode) finishMoveResize(false);
        mode = PositionCenter;
        buttonDown = TRUE;
  ...
  void Workspace::slotActivateAttentionWindow() {
    if(attention_chain.count() > 0) activateClient(attention_chain.first());
  }
  ...
  void Workspace::slotWindowToDesktop(int i) {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(i >= 1 && i <= numberOfDesktops() && c && !c->isDesktop() &&
  ...
  void Workspace::slotWindowToScreen(int i) {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(i >= 0 && i <= numScreens() && c && !c->isDesktop() &&
        !c->isDock() && !c->isTopMenu()) {
  ...
  void Workspace::slotWindowToNextScreen() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c && !c->isDesktop() && !c->isDock() && !c->isTopMenu()) {
  ...
  */
  void Workspace::slotWindowMaximize() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) performWindowOperation(c, Options::MaximizeOp);
  }
  ...
  */
  void Workspace::slotWindowMaximizeVertical() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) performWindowOperation(c, Options::VMaximizeOp);
  }
  ...
  */
  void Workspace::slotWindowMaximizeHorizontal() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) performWindowOperation(c, Options::HMaximizeOp);
  }
  ...
  */
  void Workspace::slotWindowRaiseOrLower() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) raiseOrLowerClient(c);
  }

  void Workspace::slotWindowOnAllDesktops() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) c->setOnAllDesktops(!c->isOnAllDesktops());
  }

  void Workspace::slotWindowFullScreen() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) performWindowOperation(c, Options::FullScreenOp);
  }

  void Workspace::slotWindowNoBorder() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) performWindowOperation(c, Options::NoBorderOp);
  }

  void Workspace::slotWindowAbove() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) performWindowOperation(c, Options::KeepAboveOp);
  }

  void Workspace::slotWindowBelow() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) performWindowOperation(c, Options::KeepBelowOp);
  }

  void Workspace::slotSetupWindowShortcut() {
    Client* c = active_popup_client ? active_popup_client : active_client;
    if(c) performWindowOperation(c, Options::SetupWindowShortcutOp);
  }
  ...
  */
  ...
  void Workspace::showWindowMenu(const QRect &pos, Client* cl) {
    if(!kapp->authorizeKAction("kwin_rmb")) return;
    if(!cl) return;

    //NOTE: to prevent recursion
    if(active_popup_client != NULL) return;

    if(cl->isDesktop() || cl->isDock() || cl->isTopMenu()) return;
    ...
    int x = pos.left();
    int y = pos.bottom();
    if(y == pos.top()) p->exec(QPoint(x, y));
    else {
    ...
    if(active_popup == p) closeActivePopup();
  }
  ...
  */
  ...
  void Client::setShortcut(const QString& _cut) {
    ...
    if(cut.isEmpty()) return setShortcutInternal(KShortcut());
    ...
    for(QStringList::ConstIterator it = groups.begin(); it != groups.end(); ++it) {
      ...
      if(reg.search(*it) > -1)
      {
        ...
        for(unsigned int i = 0; i < list.length(); ++i) {
          ...
          if(!c.isNull()) keys.append(c);
        }
      }
    }
    for(QValueList< KShortcut >::ConstIterator it = keys.begin(); it != keys.end(); ++it) {
      // current one is in the list
      if(_shortcut == *it) return;
    }
    for(QValueList< KShortcut >::ConstIterator it = keys.begin(); it != keys.end(); ++it) {
  ...
  bool Workspace::shortcutAvailable(const KShortcut& cut, Client* ignore) const {
    ...
    for(ClientList::ConstIterator it = clients.begin(); it != clients.end(); ++it) {
      if((*it) != ignore && (*it)->shortcut() == cut) return false;
    }
    return true;
  }
} // namespace
